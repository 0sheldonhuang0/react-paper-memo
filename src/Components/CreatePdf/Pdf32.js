...
function preparePdf(uploadData, format) {
  ...
  //设置每个格子的间距
  var paperA4 = [297, 210];
  var horizonSegment = paperA4[0] / 8; //横线
  var verticalSegment = paperA4[1] / 4; //竖线
  var horizonSegmentText = paperA4[1] / 8; //横排分割
  var verticalSegmentText = paperA4[0] / 16; //竖排分割
  ...
  if (displayUploadData.length % 32 != 0) {
    let temp = displayUploadData.length % 32;
    for (let i = 0; i < 32 - temp; i++) {
  ...
  for (let i = 0; i < displayUploadData.length - 1; i = i + 32) {
    console.log(format);
    ...
    doc.setLineDash([1]);
    for (let k = 1; k < 4; k++) {
      doc.line(k * verticalSegment, 5, k * verticalSegment, 292);
    }
    for (let k = 1; k < 8; k++) {
